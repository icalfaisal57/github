...
DEBUG_MODE = True            # Set True untuk debug geometry
...
GEE_FEATURE_CONFIG = {
    ...
    'absorbing_aerosol_index': ('COPERNICUS/S5P/NRTI/L3_AER_AI', 'absorbing_aerosol_index', 1000),
}
...
# ----------------- Helper Payload (BARU) -----------------
def build_aggregated_payload(results, tanggal_dt, kota, include_tanggal_fitur=True):
    """
    Payload sesuai schema API (PayloadAggregat):
    {
      "tanggal": "YYYY-MM-DD",
      "kota": "Depok",
      "estimasi": { "Beji": 12.3, "Bojongsari": 10.2, ... },
      "rata_rata_kota": 34.56,
      "tanggal_fitur": { "Beji": {"AOD":"2025-10-31", ...}, ... }  # opsional
    }
    """
    if not results:
        return {
    ...
    estimasi_map = {r["kecamatan"]: float(r["prediksi_pm25"]) for r in results}
    rata_rata = sum(estimasi_map.values()) / len(estimasi_map)

    payload = {
        "tanggal": tanggal_dt.strftime("%Y-%m-%d"),
        "kota": kota,
        "estimasi": estimasi_map,
        "rata_rata_kota": float(rata_rata)
    }

    if include_tanggal_fitur:
        payload["tanggal_fitur"] = {r["kecamatan"]: r.get("tanggal_fitur", {}) for r in results}
    ...
    try:
        ...
        for nama, data in kecamatan_dict.items():
            if len(data['geometries']) > 1:
                ...
                merged_geometry = {'type': 'MultiPolygon', 'coordinates': all_coords}
            else:
...
def geojson_to_ee_geometry(geojson_geometry):
    ...
    try:
        ...
        if not coords:
            print("  ✗ Koordinat kosong")
            return None

        if geom_type == 'Polygon':
            if not isinstance(coords, list) or len(coords) == 0:
                print("  ✗ Format Polygon tidak valid")
                return None
            try:
            ...
            if not isinstance(coords, list) or len(coords) == 0:
                print("  ✗ Format MultiPolygon tidak valid")
                return None
            try:
                ...
                try:
                    print("  ℹ Fallback: gunakan polygon pertama saja...")
...
def get_features_from_gee(aoi, date, kecamatan_nama):
    ...
    NON_NEGATIVE_FEATURES = ['CO', 'NO2_tropo', 'O3', 'SO2', 'Wind_Speed']
